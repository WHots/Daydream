...
mod tests
{
    use super::{validate_process_peb, ProcessPeValidationError};
    use crate::core::process_ops::utils::foundation::validate_pe;
    ...
    {
        assert_eq!(validate_process_peb(std::ptr::null_mut()), Err(ProcessPeValidationError::InvalidProcessHandle));
    }


    #[test]
    fn validates_current_process_peb_and_main_image()
    {
        // SAFETY: these APIs return metadata and a pseudo-handle for the calling process.
        let process = unsafe { GetCurrentProcess() };
        // SAFETY: this API only returns the calling process identifier.
        let process_id = unsafe { GetCurrentProcessId() };
        let validation = validate_process_peb(process).expect("current process should expose a valid x64 PEB and main image");

        assert_eq!(validation.process_id, process_id);
        assert_ne!(validation.peb_address, 0);
        assert_ne!(validation.image.base_address, 0);
        assert_ne!(validation.image.image_size, 0);
        assert_ne!(validation.image.section_count, 0);
        assert!(!validation.image_path.as_os_str().is_empty());

        let snapshot = validate_pe::read_validated_image(process, &validation.image).expect("current process main image should produce a validated snapshot");

        assert_eq!(snapshot.bytes.len(), validation.image.image_size);
        assert_eq!(snapshot.pe.sections.len(), validation.image.section_count as usize);
    }
}
