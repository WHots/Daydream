...
mod tests
{
    use super::{scan_code_signatures, scan_file_signatures, FileSignatureHit};
    ...
    #[test]
    fn collects_all_overlapping_wildcard_signature_matches_across_executable_sections()
    {
        let mut file = test_file(vec![test_section(".text", 0x1000, 0x20, 7), test_section(".code", 0x2000, 0x30, 7)], 0x1000);
        file.bytes[0x20..0x27].copy_from_slice(&[0x48, 0x11, 0xC3, 0x48, 0x22, 0xC3, 0x90]);
        file.bytes[0x30..0x37].copy_from_slice(&[0x48, 0x33, 0xC3, 0x48, 0x44, 0xC3, 0x90]);
    ...
    fn scans_catalog_signatures_with_wildcard_bytes()
    {
        ...
        file.bytes[0x20..0x2B].copy_from_slice(&[0x4C, 0x8B, 0xD1, 0xB8, 0x34, 0x12, 0x00, 0x00, 0x0F, 0x05, 0xC3]);

        let matches = scan_file_signatures(&file);

        assert_eq!(
            matches,
            vec![FileSignatureHit {
    ...
    fn scans_contextual_runtime_and_anti_analysis_signatures()
    {
        ...
        file.bytes[0x20..0x63].copy_from_slice(&[0x65, 0x48, 0x8B, 0x04, 0x25, 0x60, 0x00, 0x00, 0x00, 0x48, 0x8B, 0x40, 0x18, 0x48, 0x8B, 0x40, 0x20, 0x90, 0x65, 0x48, 0x8B, 0x04, 0x25, 0x60, 0x00, 0x00, 0x00, 0x8B, 0x80, 0xBC, 0x00, 0x00, 0x00, 0x83, 0xE0, 0x70, 0x90, 0x0F, 0xB6, 0x04, 0x25, 0xD4, 0x02, 0xFE, 0x7F, 0x90, 0x4C, 0x8B, 0xD1, 0xB8, 0x34, 0x12, 0x00, 0x00, 0xF6, 0x04, 0x25, 0x08, 0x03, 0xFE, 0x7F, 0x01, 0x75, 0x05, 0x0F, 0x05, 0xC3]);
    ...
    fn scans_only_the_designated_file_detection_signatures()
    {
        let matches = {
            ...
            file.bytes[0x20..0x37].copy_from_slice(&[0x33, 0xC0, 0xCD, 0x2D, 0xC3, 0x48, 0x83, 0xEC, 0x28, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x48, 0x83, 0xC4, 0x28, 0xE9, 0x00, 0x00, 0x00, 0x00]);
    ...
    {
        let byte_count = sections.iter().map(|section| section.raw_offset + section.raw_size).max().unwrap_or(0x20).max(0x20);
