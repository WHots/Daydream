...
//! `X64_FILE_SCAN_SIGNATURES` feeds raw-file scans. Keep descriptive category slices and
//! `X64_ANALYST_SIGNATURES` synchronized so deliberate category/all-signature callers see it.
...
pub const PEB_BEING_DEBUGGED_CHECK: Signature = x64_signature!(
    "PEB BeingDebugged check",
    [0x65, 0x48, 0x8B, 0x04, 0x25, 0x60, 0x00, 0x00, 0x00, 0x0F, 0xB6, 0x40, 0x02]
);
...
pub const NT_GLOBAL_FLAG_HEAP_DEBUG_CHECK: Signature = x64_signature!(
    "PEB NtGlobalFlag heap-debug check",
    [
        0x65, 0x48, 0x8B, 0x04, 0x25, 0x60, 0x00, 0x00, 0x00,
        0x8B, 0x80, 0xBC, 0x00, 0x00, 0x00, 0x83, 0xE0, 0x70,
    ]
);
...
pub const KD_DEBUGGER_ENABLED_READ: Signature = x64_signature!(
    "KUSER_SHARED_DATA KdDebuggerEnabled read",
    [0x0F, 0xB6, 0x04, 0x25, 0xD4, 0x02, 0xFE, 0x7F]
);
...
///   65 48 8B 04 25 60 00 00 00  mov rax, gs:[60h]
pub const PEB_RESOLVER_ANCHOR: Signature = x64_signature!(
    "PEB resolver anchor",
    [0x65, 0x48, 0x8B, 0x04, 0x25, 0x60, 0x00, 0x00, 0x00]
);
...
pub const PEB_LDR_IN_MEMORY_ORDER_WALK: Signature = x64_signature!(
    "PEB loader InMemoryOrder walk",
    [
        0x65, 0x48, 0x8B, 0x04, 0x25, 0x60, 0x00, 0x00, 0x00,
...
///   48 0B C2              or rax, rdx
pub const RDTSC_TIMESTAMP_COMBINE: Signature = x64_signature!(
    "RDTSC timestamp combine",
    [0x0F, 0x31, 0x48, 0xC1, 0xE2, 0x20, 0x48, 0x0B, 0xC2]
);
...
///   48 C1 E2 20           shl rdx, 20h
///   48 0B C2              or rax, rdx
pub const RDTSCP_TIMESTAMP_COMBINE: Signature = x64_signature!(
    "RDTSCP timestamp combine",
    [0x0F, 0x01, 0xF9, 0x48, 0xC1, 0xE2, 0x20, 0x48, 0x0B, 0xC2]
);
...
///   33 C0                 xor eax, eax
///   CD 2D                 int 2dh
pub const INT_2D_ANTI_DEBUG_CHECK: Signature = x64_signature!(
    "INT 2D anti-debug check",
    [0x33, 0xC0, 0xCD, 0x2D]
);
...
/// Runtime landmarks for code that may resolve APIs dynamically or transition to a native
/// Windows syscall.
#[allow(dead_code)]
pub const X64_RUNTIME_ANCHOR_SIGNATURES: &[Signature] = &[
    SECURITY_CHECK_COOKIE,
    PEB_RESOLVER_ANCHOR,
    PEB_LDR_IN_MEMORY_ORDER_WALK,
    DIRECT_SYSCALL_STUB,
    SHARED_USER_DATA_SYSCALL_GATE,
];

/// Contextual anti-analysis signatures. Single-instruction `UD2`, `RDTSC`, `RDTSCP`, and
/// `CPUID` opcode matches belong in an instruction decoder rather than a raw byte scanner.
#[allow(dead_code)]
pub const X64_ANTI_ANALYSIS_SIGNATURES: &[Signature] = &[
    PEB_BEING_DEBUGGED_CHECK,
    NT_GLOBAL_FLAG_HEAP_DEBUG_CHECK,
    KD_DEBUGGER_ENABLED_READ,
    RDTSC_TIMESTAMP_COMBINE,
    RDTSCP_TIMESTAMP_COMBINE,
    CPUID_LEAF_QUERY,
    INT_2D_ANTI_DEBUG_CHECK,
];
...
pub const X64_FILE_SCAN_SIGNATURES: &[Signature] = &[
    PEB_BEING_DEBUGGED_CHECK,
    NT_GLOBAL_FLAG_HEAP_DEBUG_CHECK,
    KD_DEBUGGER_ENABLED_READ,
    PEB_LDR_IN_MEMORY_ORDER_WALK,
...
#[allow(dead_code)]
pub const X64_ANALYST_SIGNATURES: &[Signature] = &[
    MAIN_CRT_STARTUP,
    SECURITY_CHECK_COOKIE,
    PEB_BEING_DEBUGGED_CHECK,
    NT_GLOBAL_FLAG_HEAP_DEBUG_CHECK,
